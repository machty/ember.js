...
QUnit.skip("Use Ember.get to retrieve query params 'refreshModel' configuration when configuration lives on route", function() {
  expect(6);
...
QUnit.skip("An explicit replace:false on a changed QP always wins and causes a pushState even when configuration is all on the route", function() {
  expect(3);
...
QUnit.skip("Use Ember.get to retrieve query params 'replace' configuration when all configuration is on the route", function() {
  expect(2);
...
QUnit.skip("Subresource naming style is supported when configuration is all on the route", function() {
...
QUnit.skip("setting QP to empty string doesn't generate null in URL", function() {
  expect(1);
  App.IndexRoute = Ember.Route.extend({
    queryParams: {
      foo: "123"
...
QUnit.skip("Url with array query param sets controller property to array when configuration occurs on the route", function() {
  App.IndexRoute = Ember.Route.extend({
    queryParams: {
      foo: {
        defaultValue: ''
...
QUnit.skip("Url with array query param sets controller property to array when configuration occurs on the route and there is still a controller", function() {
  App.IndexController = Ember.Controller.extend();
...
QUnit.skip("Array query params can be pushed/popped when configuration occurs on the route but there is still a controller", function() {
  Router.map(function() {
    this.route("home", { path: '/' });
  });
...
QUnit.skip("Overwriting with array with same content shouldn't refire update when configuration occurs on router but there is still a controller", function() {
  expect(3);
  var modelCount = 0;
...
QUnit.skip("query params have been set by the time setupController is called when configuration occurs on the router and there is still a controller", function() {
  expect(1);
...
var testParamlessLinksWithRouteConfig = function(routeName) {
  QUnit.skip("param-less links in an app booted with query params in the URL don't reset the query params: " + routeName, function() {
    expect(1);
...
QUnit.test("Support shared but overridable mixin pattern", function() {
  ...
  App.ParentRoute = Ember.Route.extend(HasPage, {
    queryParams: {
      page: {
        as: 'yespage'
      }
  ...
  App.ParentChildRoute = Ember.Route.extend(HasPage);
